...
import {
    ModelPredictionUsingChap
} from '../../src/services/ModelPredictionUsingChap';
...
describe('POST /synthetic-predict',function() {
    ...
    beforeEach(() => {
        ...
        modelPredictStub = sinon.stub(ModelPredictionUsingChap.prototype, 'predict')
            .resolves({
                status: 'success'
            });
        modelGetStatusStub = sinon.stub(ModelPredictionUsingChap.prototype, 'getStatus')
            .resolves({
                status: 'idle',
                ready: true
            });

        modelGetResultStub = sinon.stub(ModelPredictionUsingChap.prototype, 'getResult')
            .resolves({
                data: {
                    dataValues: [{
                        ou: 'test_ou',
    ...
    it('should process valid CSV files and return 201', async () => {
        ...
        sinon.assert.calledWithMatch(uploadToMinioStub, 
            sinon.match.string, 
